...
public:   
   ...
   public:    
      ...
      void HWLIB_INLINE write_and_read( 
         ...
         bus.write_and_read( n, data_out.begin());
      }   
...
private:
   ...
   void write_and_read( 
      ...
      const uint8_t data_out[]
         ...
         uint32_t clock = (0x1U << 28);
         uint32_t data = (0x1U << 26);

         for( uint_fast8_t j = 0; j < 8; ++j ){ 

            //mosi.write( ( d & 0x80 ) != 0 ); 
            if((d & 0x80) != 0 ){
               PIOC->PIO_SODR = data; // write(1);
            } else {
               PIOC->PIO_CODR = data; // write(0);
            }  
         
            //sclk.write( 1 );
            PIOC->PIO_CODR = clock; //0x2000000; write(0);

            d = d << 1;

            PIOC->PIO_SODR = clock; //0x2000000; write(1);
            //sclk.write( 0 );   
         }                           
   }      
